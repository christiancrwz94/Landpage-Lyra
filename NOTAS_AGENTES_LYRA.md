...
## Cuidado com Restaura��o de Arquivos (git checkout)
� ESTRITAMENTE PROIBIDO utilizar git checkout HEAD <arquivo> (ou similares) de forma leviana para desfazer pequenos erros de edi��o durante uma sess�o. O uso desse comando apaga silenciosamente todas as modifica��es n�o commitadas (tanto as feitas pelo pr�prio usu�rio quanto por outros agentes na mesma sess�o), o que resulta em perda irrepar�vel de c�digo (como mockups injetados, scripts gerados dinamicamente e atualiza��es de CSS). Se precisar reverter uma altera��o pontual, recupere o hist�rico via transcript ou use ferramentas de replace focadas. NUNCA sobrescreva a �rvore de trabalho inteira do arquivo se n�o tiver 100% de certeza absoluta de que n�o h� trabalho n�o commitado nele.


## Manipula��o Segura de Uploads de Imagens
Ao copiar arquivos enviados pelo usu�rio da pasta .user_uploaded, � ESTRITAMENTE PROIBIDO filtrar por extens�es espec�ficas (como *.png) ao buscar o arquivo mais recente. O usu�rio pode enviar JPGs, WebPs ou PNGs. O filtro restritivo causa bugs ao pular o arquivo real e selecionar acidentalmente capturas de tela antigas. Sempre liste TODOS os arquivos do diret�rio, ordene pelo LastWriteTime mais recente e apenas depois valide a extens�o do arquivo retornado.
